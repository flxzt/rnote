...
mod imp {
    ...
    impl RnCanvas {
        fn setup_input(&self) {
            ...
            let stylus_in_proximity = Cell::new(false);
            self.pointer_controller.connect_event(clone!(
                ...
                move |_, event| {
                    let (propagation, new_state, new_stylus_in_proximity) =
                        super::input::handle_pointer_controller_event(
                            &canvas,
                            event,
                            pen_state.get(),
                            stylus_in_proximity.get(),
                        );
                    pen_state.set(new_state);
                    stylus_in_proximity.set(new_stylus_in_proximity);
                    propagation
                }
            ));
