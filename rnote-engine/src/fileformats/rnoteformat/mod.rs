...
use self::maj0min7::RnoteFileMaj0Min7;
...
impl FileFormatLoader for RnoteFile {
    fn load_from_bytes(bytes: &[u8]) -> anyhow::Result<Self> {
        ...
        if semver::VersionReq::parse(">=0.7.0")
            .unwrap()
            .matches(&wrapper.version)
        {
            serde_json::from_value::<RnoteFileMaj0Min7>(wrapper.data)
                .context("from_value() for newest file version failed")
        } else if semver::VersionReq::parse(">=0.5.10")
            .unwrap()
            .matches(&wrapper.version)
        {
            RnoteFileMaj0Min7::try_from(
                serde_json::from_value::<RnoteFileMaj0Min6>(wrapper.data)
                    .context("from_value() for RnoteFileMaj0Min6 failed")?,
            )
            ...
            .matches(&wrapper.version)
        {
            ...
            .and_then(RnoteFileMaj0Min7::try_from)
