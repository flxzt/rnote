...
use gtk4::{
    gdk, gio, glib, prelude::*, subclass::prelude::*, Button, CompositeTemplate, DirectoryList,
    Grid, ListBox, ListBoxRow, ListView, ScrolledWindow, Widget,
};
...
impl WorkspaceBrowser {
    ...
    pub fn init(&self, appwindow: &RnoteAppWindow) {
        let remove_workspace_button = components::setup_remove_workspace_button(self, appwindow);
        let edit_workspace_button = components::setup_edit_workspace_button(self, appwindow);
        let _add_workspace_button = components::setup_add_workspace_button(self, appwindow);

        components::setup_workspace_list(
            self,
            appwindow,
            &remove_workspace_button,
            &edit_workspace_button,
        );
        components::setup_workspace_listbox(self, appwindow);
        components::setup_files_prefix_listbox(self, appwindow);
        components::setup_file_rows(self, appwindow);
